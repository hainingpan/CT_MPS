...
function run_dw_t(L::Int,p_ctrl::Float64,p_proj::Float64,seed_C::Int,seed_m::Int)
    ct=CT.CT_MPS(L=L,seed=0,seed_C=seed_C,seed_m=seed_m,folded=true,store_op=false,store_vec=false,ancilla=0,xj=Set([0]),x0=x0=1//2^L )
    print("x0: ", ct.x0)
    ...
    fdw=zeros(tf+1,L+1)
    fdw[1,:] = CT.fdw_weights(ct,i)
    # println("finished initialization")
    
    for idx in 1:tf
        println(idx)
        i=CT.random_control!(ct,i,p_ctrl,p_proj)
        # println("finished random control: ", i)
        fdw[idx+1,:] = CT.fdw_weights(ct,i)
    end
    return Dict("fdw"=>fdw,)
end
...
function main_interactive(L::Int,p_ctrl::Float64,p_proj::Float64,seed_C::Int,seed_m::Int)
    ...
    filename = "MPS_(0,1)_L$(L)_pctrl$(@sprintf("%.3f", p_ctrl))_pproj$(@sprintf("%.3f", p_proj))_sC$(seed_C)_sm$(seed_m)_coherence_T.json"
...
# julia run_CT_MPS_C_m_FDW_T.jl --p_ctrl 0.5 --p_proj 0.0 --L 20 --seed_C 0 --seed_m 0