...
function run_Oi_t(L::Int,p_ctrl::Float64,p_proj::Float64,seed_C::Int,seed_m::Int)
    ...
    Oi_list[1,:]=circshift(CT.Zi(ct),0)
    
    for idx in 1:tf
        i=CT.random_control!(ct,i,p_ctrl,p_proj)
        # Oi_list[idx+1,:]=collect(CT.dw(ct,(i%ct.L)+1))
        # O_list[idx+1,:]=[CT.Z(ct),CT.Z_sq(ct)]
        # Oi_list[idx+1,:]=circshift(CT.Zi(ct),-i)
        Oi_list[idx+1,:]=circshift(CT.Zi(ct),0)
    end
    # O1=CT.Z(ct)
    # O2=CT.Z_sq(ct)
    ...
    # return Dict("DW1"=>Oi_list[:,1],"DW2"=>Oi_list[:,2])
    # return Dict("O1"=>O_list[:,1],"O2"=>O_list[:,2])
    return Dict("Oi"=>Oi_list,"op_history"=>ct.op_history)
end
...
function main()
    ...
    args = parse_my_args()
    results = run_Oi_t(args["L"], args["p_ctrl"], args["p_proj"], args["seed_C"],args["seed_m"])

    filename = "MPS_(0,1)_L$(args["L"])_pctrl$(@sprintf("%.3f", args["p_ctrl"]))_pproj$(@sprintf("%.3f", args["p_proj"]))_sC$(args["seed_C"])_sm$(args["seed_m"])_x01_Oi_T.json"
...
function main_interactive(L::Int,p_ctrl::Float64,p_proj::Float64,seed_C::Int,seed_m::Int)
    ...
    filename = "MPS_(0,1)_L$(L)_pctrl$(@sprintf("%.3f", p_ctrl))_pproj$(@sprintf("%.3f", p_proj))_sC$(seed_C)_sm$(seed_m)_x01_Oi_T.json"
    # filename = "MPS_(0,1)_L$(L)_pctrl$(@sprintf("%.3f", p_ctrl))_pproj$(@sprintf("%.3f", p_proj))_sC$(seed_C)_sm$(seed_m)_Oi_T.json"
    ...
    # else
        results = run_Oi_t(L, p_ctrl, p_proj, seed_C,seed_m)
...
# julia run_CT_MPS_C_m_O_T.jl --p_ctrl 0.5 --p_proj 0.0 --L 10 --seed_C 0 --seed_m 0