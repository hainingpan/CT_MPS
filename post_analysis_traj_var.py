...
def run(L):
    ...
    #     # single=[data.xs(sm,level='sm').loc['DW1']['observations'] for sm in range((p1arams_list[0][1]['sm']).shape[0])]
    ...
    # for p in tqdm(params_list[0][1]['p_proj']):
        for L in params_list[0][1]['L']:
            print(p,L)
            # sC_traj_var=[trajvar(df_MPS_0_T,L=L,p_ctrl=p,sC=sC) for sC in range((params_list[0][1]['sC']).shape[0])]
            sC_traj_var[(p,L)]=[]
            for sC in range((params_list[0][1]['sC']).shape[0]):
                try:
                    sC_traj_var[(p,L)].append(trajvar(df_MPS_0_T,L=L,p_ctrl=p,sC=sC,p_proj=0))
                except:
                    pass
            sC_traj_var[(p,L)]=np.array(sC_traj_var[(p,L)])
            traj_var_dict[(p,L)], traj_var_sem_dict[(p,L)]=np.mean(sC_traj_var[(p,L)],axis=0), np.std(sC_traj_var[(p,L)],axis=0)/np.sqrt((params_list[0][1]['sC']).shape[0])
    ...
    with open(f'traj_var_{ob}_L{L}.pickle','wb') as f:
        pickle.dump([sC_traj_var,traj_var_dict,traj_var_sem_dict],f)
